...
const GeneratedElements& ElementsGenerator::generateElements(const std::vector<sf::Vector2f>& walkablePositions, const sf::Texture& atlasTexture)
{
	...
	m_generatedElements.obstacles	= generateObstacles(gen, atlasTexture);
	m_generatedElements.keys		= generateKeys(gen, atlasTexture);
	m_generatedElements.enemies		= generateEnemies(gen, atlasTexture);
...
std::vector <std::shared_ptr<Obstacle>> ElementsGenerator::generateObstacles(std::mt19937& gen, const sf::Texture& atlasTexture)
{
	...
	for (auto it = freePositions.begin(); it != freePositions.end();)
	{
		...
		auto obstacle = std::make_shared<Obstacle>(Obstacle(ObstacleType::Breakable, { x, y }, false));

		// Define, load, assign and set the specific texture and set the position of the obstacle in the game world
		obstacle->setTexture(atlasTexture);
...
std::vector<std::shared_ptr<Key>> ElementsGenerator::generateKeys(std::mt19937& gen, const sf::Texture& atlasTexture)
{
	...
	for (auto it = breakableObstaclesPositions.begin(); it != breakableObstaclesPositions.end();)
	{
		...
		auto key = std::make_shared<Key>(Key({ x,y }));
...
std::vector<std::shared_ptr<Gate>> ElementsGenerator::generateGates(std::mt19937& gen, const sf::Texture& atlasTexture)
{
	...
	for (auto it = breakableObstaclesPositions.begin(); it != breakableObstaclesPositions.end(); ++it)
	{
		...
		// Create a new breakable obstacle at the current position
		auto gate = std::make_shared<Gate>(Gate({x,y}, false, m_generatedElements.keys[0].get()));
...
std::vector <std::shared_ptr<Enemy>> ElementsGenerator::generateEnemies(std::mt19937& gen, const sf::Texture& atlasTexture)
{
	//temp enemies
	...
	for (auto it = freePositions.begin(); it != freePositions.end();)
	{
		...
		auto enemy = std::make_shared<Enemy>(Enemy(EnemyType::Basic, { (float)x, (float)y }));

		// Define, load, assign and set the specific texture and set the position of the obstacle in the game world
		enemy->setTexture(atlasTexture);
...
std::vector<sf::Vector2i> ElementsGenerator::generatePatrollingPoints(std::mt19937& gen, const sf::Vector2i& enemyPosition, std::vector<sf::Vector2f>& newFreePositions, int32_t range) const
{
...
void ElementsGenerator::draw(sf::RenderTarget& target) const
{
...
void ElementsGenerator::update(sf::RenderTarget& target)
{
