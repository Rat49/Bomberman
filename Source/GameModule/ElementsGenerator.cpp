...
const GeneratedElements& ElementsGenerator::generateElements(const std::vector<sf::Vector2f>& walkablePositions,
...
std::vector<std::shared_ptr<Obstacle>> ElementsGenerator::generateObstacles(std::mt19937& gen)
{
    ...
    freePositions.erase(std::remove_if(freePositions.begin(),
                                       freePositions.end(),
                                       [&usedPositions](const sf::Vector2f& pos)
                                       {
...
std::vector<std::shared_ptr<Gate>> ElementsGenerator::generateGates(std::mt19937& gen, const sf::Texture& atlasTexture)
{
    ...
    for (auto it = breakableObstaclesPositions.begin(); it != breakableObstaclesPositions.end(); ++it)
    {
        ...
        auto found = std::find_if(breakableObstaclesPositions.begin(),
...
std::vector<std::shared_ptr<EnemyBase>> ElementsGenerator::generateEnemies(std::mt19937& gen)
{
    ...
    // Remove positions that overlap with the safety zone
    freePositions
        .erase(std::remove_if(freePositions.begin(),
                              freePositions.end(),
                              [&usedPositions](const sf::Vector2f& pos)
                              {
                                  std::pair<int32_t, int32_t> positionPair = std::make_pair(static_cast<int32_t>(pos.x),
    ...
    for (auto it = freePositions.begin(); it != freePositions.end();)
    {
        ...
        {
            case 1:
                enemy = EnemyFactory::createEnemy(EnemyType::Basic,
                                                  sf::Vector2f(static_cast<float>(x), static_cast<float>(y)),
                                                  grid);
                break;
            case 2:
                enemy = EnemyFactory::createEnemy(EnemyType::Medium,
                                                  sf::Vector2f(static_cast<float>(x), static_cast<float>(y)),
                                                  grid);
                break;
            case 3:
                enemy = EnemyFactory::createEnemy(EnemyType::Hard,
                                                  sf::Vector2f(static_cast<float>(x), static_cast<float>(y)),
                                                  grid);
                break;
            default:
