...
void UIInputField::setPosition(const sf::Vector2f& pos)
{
    ...
    m_inputText.setPosition(pos.x + INPUT_TEXT_OFFSET,
                            pos.y + (m_fieldBackground.getSize().y - m_inputText.getCharacterSize()) / 2);

    //position of cursor
    sf::FloatRect textBounds = m_inputText.getGlobalBounds();
    m_cursor.setPosition(textBounds.left + textBounds.width + CURSOR_OFFSET,
                         m_fieldBackground.getPosition().y +
                             (m_fieldBackground.getSize().y - m_inputText.getCharacterSize()) / 2);
}
...
bool UIInputField::isFieldEnd()
{
    ...
    if (textBounds.width > m_fieldBackground.getSize().x -
                               (m_characterSizeOriginal + m_cursor.getSize().x + CURSOR_OFFSET + INPUT_TEXT_OFFSET))
    {
