...
void UIFactory::setUIScreen(UIScreen* screen, const ConfigSection& element)
{
    ...
    if (element.areValuesPresent({WIDTH_OFFSET, HEIGHT_OFFSET, RECTANGLE_HEIGHT, RECTANGLE_WIDTH, RECTANGLE_COLOR}))
    {
        ...
        screen->setBackground(elementWidthOffset * width,
                              elementHeightOffset * height,
                              elementWidth,
                              elementHeight,
...
void UIFactory::makeUIInputField(UIScreen* screen, const std::string& screenFont, const ConfigSection& element)
{
    ...
    if (element.areValuesPresent({NAME, WIDTH_OFFSET, HEIGHT_OFFSET, RECTANGLE_WIDTH, RECTANGLE_HEIGHT}))
    {
        ...
        if (element.areValuesPresent({TEXT_COLOR, FIELD_COLOR}))
        {
            ...
            myInputField->setColor(sf::Color(std::stoul(fieldColor, nullptr, 16)),
                                   sf::Color(std::stoul(textColor, nullptr, 16)));
        }
