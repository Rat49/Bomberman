...
void InputModule::LoadInputSettings(const std::string& inputSettingPath)
{
    ...
    {
        if (!inputSettings.isSectionPresent(section))
            break;
        ...
        else if (inputSettings.getSection(section).areValuesPresent(
                     {"type", "negativeAxisButton", "positiveAxisButton"}))
        {
            Axis1D axis1D           = {};
            axis1D.negativeAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("negativeAxisButton").getString());
            axis1D.positiveAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("positiveAxisButton").getString());
        ...
        {
            Axis2D axis2D                      = {};
            axis2D.Horizontal.negativeAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("negativeXAxisButton").getString());
            axis2D.Horizontal.positiveAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("positiveXAxisButton").getString());
            axis2D.Vertical.negativeAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("negativeYAxisButton").getString());
            axis2D.Vertical.positiveAxis.Key = inputBinder.GetButton(
                inputSettings.getSection(section).getValue("positiveYAxisButton").getString());
...
void InputModule::update(float, sf::Window*)
{
    for (auto& action : actions)
    {
        ...
        switch (actionType)
        {
            case EActionType::ButtonAction:
                ...
                shouldEmit = ((shouldEmit && action.second.State.buttonState &&
                               !action.second.Binding.button.previousButtonState) ||
                              (shouldEmit && !action.second.State.buttonState &&
                               action.second.Binding.button.previousButtonState));
            ...
            case EActionType::Axis1DAction:
                action.second.State
                    .axis1DState = (sf::Keyboard::isKeyPressed(action.second.Binding.axis1D.negativeAxis.Key) ? -1.0f : 0.0f) +
                                   (sf::Keyboard::isKeyPressed(action.second.Binding.axis1D.positiveAxis.Key) ? 1.0f : 0.0f);
            ...
            case EActionType::Axis2DAction:
                action.second.State.axis2DState
                    .x = (sf::Keyboard::isKeyPressed(action.second.Binding.axis2D.Horizontal.negativeAxis.Key) ? -1.0f : 0.0f) +
                         (sf::Keyboard::isKeyPressed(action.second.Binding.axis2D.Horizontal.positiveAxis.Key) ? 1.0f : 0.0f);
                action.second.State.axis2DState
                    .y = (sf::Keyboard::isKeyPressed(action.second.Binding.axis2D.Vertical.negativeAxis.Key) ? 1.0f : 0.0f) +
                         (sf::Keyboard::isKeyPressed(action.second.Binding.axis2D.Vertical.positiveAxis.Key) ? -1.0f : 0.0f);
        ...
        if (shouldEmit)
            Modules::Events->emit(eventID, params);
    }
}

void InputModule::updateBinding(const std::string& actionName,
                                const std::string& actionType,
                                const std::string& newKey,
                                const std::string& actionAxis)
{
...
void InputModule::terminate()
{
    for (auto it = actions.begin(); it != actions.end(); ++it)
    {
        if (it->second.eventID != -1)
        {
            LOG("Action $ with ID: $, event wasn't unregisterd, EventID: $ ",
                it->second.actionName,
                it->first,
                it->second.eventID);
        }
